...
{
    public class TcpSocketInboxTests
    {
        ...
        {
            var configuration = new TestTcpSocketFeederConfiguration();
        ...
        public async Task ReceiveAsync_ClaimedByAnotherOwner_ShouldNotInvokeTheHandler()
        {
            ...
            storeFactory.GetStore("tcpsocket-store", InboxStoreType.InMemory).Returns(store);
            var options = new InboxOptions { InboxEnabled = true, StoreType = InboxStoreType.InMemory, StoreConnectionName = "tcpsocket-store" };
        ...
        private sealed class TestTcpSocketFeederConfiguration : TcpSocketFeederConfiguration;
    }
}
