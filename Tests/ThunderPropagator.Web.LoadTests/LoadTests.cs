...
namespace ThunderPropagator.Web.LoadTests
{
        ...
        class LoadTests : IClassFixture<WebApplicationFactory<Program>>
    {
        ...
        private static ThunderPropagatorApplication? _ThunderPropagatorApplication;

        private static ThunderPropagatorApplication ThunderPropagatorApplication
        {
            get
            {
                lock (Mutex)
                    return _ThunderPropagatorApplication ??= new ThunderPropagatorApplication();
            }
        }
        ...
        {
            var httpClient = ThunderPropagatorApplication.CreateClient();

            var uri = new UriBuilder(ThunderPropagatorApplication.Server.BaseAddress)
            {
                Path = "ThunderPropagator/serverInformationModel"
            }.Uri;
        ...
        private static async Task<int> GetConnectionsCount()
        {
            ...
            using var httpClient = ThunderPropagatorApplication.CreateClient();
            var httpResponseMessage = await httpClient.GetAsync(uri, CancellationToken.None);
            var exception = Record.Exception(() => httpResponseMessage.EnsureSuccessStatusCode());
            Assert.Null(exception);
            ...
            where TWebSocketResponse : WebSocketConnectionResponse, new()
        {
            ...
            try
            {
                var webSocketUri = new UriBuilder(ThunderPropagatorApplication.Server.BaseAddress)
                {
                    Scheme = "ws",
                    Path = "ws"
                }.Uri;

                var webSocketClient = ThunderPropagatorApplication.Server.CreateWebSocketClient();
