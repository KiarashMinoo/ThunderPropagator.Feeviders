...
namespace ThunderPropagator.Feeders.Inbox
{
    ...
    /// lock" <see cref="EfCoreInboxStore.InitializeAsync"/> needs to serialize concurrent replicas'
    /// schema creation/upgrade attempts. Session-scoped (not the whole-transaction-only variant some
    /// engines also offer) uniformly across all three providers, since MySQL's <c>GET_LOCK</c>/<c>RELEASE_LOCK</c>
    /// has no transaction-scoped equivalent at all - the caller must keep the same open connection alive
    /// (<c>Database.OpenConnectionAsync</c>) for the whole acquire-work-release sequence and always call
    /// <see cref="ReleaseAsync"/>, even on failure.
    /// </summary>
    /// <remarks>
    /// PostgreSQL uses <c>pg_try_advisory_lock</c> (immediate, non-blocking) in a caller-driven retry
    /// loop, since a <see cref="TimeSpan"/> timeout has no direct equivalent for the blocking
    /// <c>pg_advisory_lock</c>. SQL Server's <c>sp_getapplock</c> and MySQL's <c>GET_LOCK</c> both support
    /// a native blocking wait with a timeout directly, so no retry loop is needed for either. Only the
    /// PostgreSQL path is exercised against a real database in this repo's own tests - see the #127
    /// commit message for the documented gap on the other two.
    /// </remarks>
    /// <remarks>
    ...
    internal static class SchemaLockDialect
    {
        ...
        private static async Task<bool> TryAcquirePostgresAsync(DbContext db, string lockName, TimeSpan timeout, CancellationToken cancellationToken)
        {
            ...
            do
            {
                // EF Core's scalar SqlQueryRaw<T> wraps the raw SQL and projects a column literally
                // named "Value" - the alias below is required, not decorative (confirmed empirically:
                // omitting it throws "column s.Value does not exist").
        ...
        private static async Task<bool> TryAcquireSqlServerAsync(DbContext db, string lockName, TimeSpan timeout, CancellationToken cancellationToken)
        {
            ...
            // sp_getapplock returns >= 0 on success (0 or 1 = granted, without/with a wait; 2/4 = other
            // granted variants), negative on failure/timeout - see sp_getapplock's own documented return codes.
            return result >= 0;
        }
        ...
        private static async Task<bool> TryAcquireMySqlAsync(DbContext db, string lockName, TimeSpan timeout, CancellationToken cancellationToken)
        {
            ...
            // GET_LOCK returns 1 on success, 0 on timeout, NULL on error (e.g. out of memory) - only 1 means acquired.
            return result == 1;
        }
    }
}
