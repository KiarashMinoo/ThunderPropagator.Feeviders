...
{
    /// <summary>
    /// Optional capability an <see cref="IOutboxStore"/> backend implements when it has a persisted
    /// schema/index/key-layout that must exist (and match a known version) before the store is safe to
    /// use. A backend with nothing to persist (<c>InMemoryOutboxStore</c>) implements it too, trivially,
    /// so callers never need to special-case "does this backend need initializing" - they just always
    /// call it.
    /// </summary>
    /// <remarks>
    ...
    /// </remarks>
    public interface IOutboxStoreInitializer
    {
        /// <summary>
        /// Ensures this store's persisted schema exists and is compatible with what this code requires.
        /// Callers should call this once per store, before starting anything (an <c>OutboxRelayWorker</c>,
        /// a direct <see cref="IOutboxStore.EnqueueAsync"/> caller) that depends on it - see
