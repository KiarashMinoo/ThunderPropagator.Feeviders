...
namespace ThunderPropagator.Providers.DotNet.Outbox
{
    ...
    /// <see cref="IOutboxDeadLetterNotificationSink"/> - "bounded" because it calls the sink at most once
    ...
    /// </summary>
    public sealed class OutboxChannelDeadLetterHandler(IOutboxDeadLetterNotificationSink sink, ILogger<OutboxChannelDeadLetterHandler> logger) : IOutboxDeadLetterHandler
    {
        /// <summary>
        /// Header key an <see cref="IOutboxDeadLetterNotificationSink"/> must stamp on any message it
        /// sends as the notification itself, if that message can re-enter this same dead-letter
        /// machinery. Its presence on <see cref="OutboxDeadLetterContext.Headers"/> tells this handler to
        ...
        public async ValueTask<OutboxDeadLetterHandlerOutcome> HandleAsync(OutboxDeadLetterContext context, CancellationToken cancellationToken)
        {
            ...
            if (context.Headers.ContainsKey(RecursionGuardHeader))
            {
                ...
                return OutboxDeadLetterHandlerOutcome.Skipped;
            }
