...
{
    /// <summary>
    /// Persists/restores W3C trace context through <see cref="OutboxMessage.Headers"/>, so an
    /// asynchronous relay attempt (possibly after a process restart, possibly minutes or hours later) can
    /// still be correlated back to the trace that enqueued the message - as an <see cref="ActivityLink"/>,
    /// never as a synchronous parent span, since a relay is not a child of an operation that already
    /// finished.
    /// </summary>
    internal static class OutboxTraceContext
    {
        ...
        /// under <see cref="OutboxHeaderNames.TraceParent"/>/<see cref="OutboxHeaderNames.TraceState"/> -
        ...
        internal static IReadOnlyDictionary<string, string>? WithCurrentTraceContext(IReadOnlyDictionary<string, string>? headers)
        {
            ...
            merged[OutboxHeaderNames.TraceParent] = current.Id!;

            if (!string.IsNullOrEmpty(current.TraceStateString))
                merged[OutboxHeaderNames.TraceState] = current.TraceStateString;

            return merged;
        }

        /// <summary>
        /// Parses a previously-persisted <see cref="OutboxHeaderNames.TraceParent"/>/<see cref="OutboxHeaderNames.TraceState"/>
        ...
        {
            if (headers.TryGetValue(OutboxHeaderNames.TraceParent, out var traceParent)
                && ActivityContext.TryParse(traceParent, headers.GetValueOrDefault(OutboxHeaderNames.TraceState), out var context))
            {
