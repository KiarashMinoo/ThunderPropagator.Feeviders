...
{
    /// <summary>
    /// Computes the delay before a relay worker next attempts a Failed entry - full jitter over an
    /// exponential curve (<c>attempt-1</c> doublings of <see cref="OutboxOptions.RetryBaseDelay"/>,
    /// capped at <see cref="OutboxOptions.RetryMaxDelay"/>, then a uniformly random delay somewhere in
    ...
    {
        /// <param name="options">Supplies <see cref="OutboxOptions.RetryBaseDelay"/>/<see cref="OutboxOptions.RetryMaxDelay"/>.</param>
        /// <param name="attemptCount">The entry's <see cref="OutboxMessage.Attempts"/> after the attempt that just failed. Values below 1 are treated as 1.</param>
        /// <param name="random">
        ...
        /// </param>
        public static TimeSpan Compute(OutboxOptions options, int attemptCount, Random random)
        {
