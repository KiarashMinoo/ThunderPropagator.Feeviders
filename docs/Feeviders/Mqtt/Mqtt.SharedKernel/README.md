...
The **Mqtt.SharedKernel** area groups 1 documented type, including `MqttFeeviderConfiguration`. It provides the contracts and implementation used by this part of ThunderPropagator.Feeviders.
...
| `MqttFeeviderConfiguration.cs` | `MqttFeeviderConfiguration` | 407 | Defines MqttFeeviderConfiguration and its related behavior. |
| `MqttTelemetry.cs` | `MqttTelemetry` | 37 | Defines MqttTelemetry and its related behavior. |
| `ThunderPropagator.Feeviders.Mqtt.SharedKernel.csproj` | — | 15 | Defines project build targets, dependencies, and package metadata. |

## Types and Members

| Type | Kind | Summary | Inherits/Implements | Key Members |
|---|---|---|---|---|
| [`MqttFeeviderConfiguration`](#mqttfeeviderconfiguration) | class | Represents the MqttFeeviderConfiguration class. | `ServiceConfiguration` | — |
...
- **Summary:** Represents the MqttFeeviderConfiguration class.
- **Thread safety:** Follow the lifetime and concurrency guarantees of the owning component; no additional guarantee is inferred.

**Usage recipe**

```csharp
// Resolve MqttFeeviderConfiguration from the configured service container or construct it with its declared dependencies.
```
...
```mermaid
graph TD
  Current["Mqtt.SharedKernel"]
  Current --> T0["MqttFeeviderConfiguration"]
```

The diagram shows the direct components documented by the **Mqtt.SharedKernel** area.

## Examples

Start with `MqttFeeviderConfiguration` as the primary entry point for this folder, then follow its linked contracts and collaborators.
