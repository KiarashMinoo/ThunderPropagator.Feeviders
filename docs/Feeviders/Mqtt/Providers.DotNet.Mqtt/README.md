...
The **Providers.DotNet.Mqtt** area groups 3 documented types, including `MqttProviderConfiguration`, `MqttProviderExtensions`, `MqttProviderMessage`. It provides the contracts and implementation used by this part of ThunderPropagator.Feeviders.
...
| `MqttProvider.cs` | `MqttProvider`, `Log` | 116 | Defines MqttProvider, Log and its related behavior. |
| `MqttProviderConfiguration.cs` | `MqttProviderConfiguration` | 16 | Defines MqttProviderConfiguration and its related behavior. |
| `MqttProviderExtensions.cs` | `MqttProviderExtensions` | 27 | Defines MqttProviderExtensions and its related behavior. |
| `MqttProviderMessage.cs` | `MqttProviderMessage` | 11 | Defines MqttProviderMessage and its related behavior. |
| `ThunderPropagator.Providers.DotNet.Mqtt.csproj` | — | 8 | Defines project build targets, dependencies, and package metadata. |

## Types and Members

| Type | Kind | Summary | Inherits/Implements | Key Members |
|---|---|---|---|---|
| [`MqttProviderConfiguration`](#mqttproviderconfiguration) | class | Represents the MqttProviderConfiguration class. | `MqttFeeviderConfiguration, IAbstractProviderConfiguration` | — |
| [`MqttProviderExtensions`](#mqttproviderextensions) | class | Represents the MqttProviderExtensions class. | — | — |
| [`MqttProviderMessage`](#mqttprovidermessage) | class | Represents the MqttProviderMessage class. | `FeederMessage` | `MqttProviderKey` |

### MqttProviderConfiguration

- **Kind:** class
- **Namespace:** `ThunderPropagator.Providers.DotNet.Mqtt`
- **Inherits/implements:** `MqttFeeviderConfiguration, IAbstractProviderConfiguration`
- **Attributes:** None detected
- **Key members:** Refer to the API surface in the source package
- **Summary:** Represents the MqttProviderConfiguration class.
- **Thread safety:** Follow the lifetime and concurrency guarantees of the owning component; no additional guarantee is inferred.

**Usage recipe**

```csharp
// Resolve MqttProviderConfiguration from the configured service container or construct it with its declared dependencies.
```

[↑ Back to top](#contents)

### MqttProviderExtensions
...
- **Attributes:** None detected
- **Key members:** Refer to the API surface in the source package
- **Summary:** Represents the MqttProviderExtensions class.
- **Thread safety:** Follow the lifetime and concurrency guarantees of the owning component; no additional guarantee is inferred.

**Usage recipe**

```csharp
// Resolve MqttProviderExtensions from the configured service container or construct it with its declared dependencies.
```
...
- **Attributes:** None detected
- **Key members:** `MqttProviderKey`
- **Summary:** Represents the MqttProviderMessage class.
- **Thread safety:** Follow the lifetime and concurrency guarantees of the owning component; no additional guarantee is inferred.

**Usage recipe**

```csharp
// Resolve MqttProviderMessage from the configured service container or construct it with its declared dependencies.
```
...
```mermaid
graph TD
  Current["Providers.DotNet.Mqtt"]
  Current --> T0["MqttProviderConfiguration"]
  Current --> T1["MqttProviderExtensions"]
  Current --> T2["MqttProviderMessage"]
```

The diagram shows the direct components documented by the **Providers.DotNet.Mqtt** area.

## Examples

Start with `MqttProviderConfiguration` as the primary entry point for this folder, then follow its linked contracts and collaborators.

## See Also

- [Parent area](../README.md)
- [Feeders.Mqtt](../Feeders.Mqtt/README.md)
