...
The **ActiveMQ.SharedKernel** area groups 1 documented type, including `IActiveMQFeeviderConfiguration`. It provides the contracts and implementation used by this part of ThunderPropagator.Feeviders.

## Files

| File | Primary type(s)/symbol(s) | LOC (approx.) | Responsibility |
|---|---|---:|---|
| `ActiveMQFeeviderConnectionFactory.cs` | `ActiveMQFeeviderConnectionFactory` | 100 | Defines ActiveMQFeeviderConnectionFactory and its related behavior. |
| `ActiveMQTelemetry.cs` | `ActiveMQTelemetry` | 29 | Defines ActiveMQTelemetry and its related behavior. |
| `AssemblyInfo.cs` | — | 5 | Contains the assembly info implementation or configuration. |
| `IActiveMQFeeviderConfiguration.cs` | `IActiveMQFeeviderConfiguration` | 36 | Defines IActiveMQFeeviderConfiguration and its related behavior. |
| `ThunderPropagator.Feeviders.ActiveMQ.SharedKernel.csproj` | — | 15 | Defines project build targets, dependencies, and package metadata. |

## Types and Members

| Type | Kind | Summary | Inherits/Implements | Key Members |
|---|---|---|---|---|
| [`IActiveMQFeeviderConfiguration`](#iactivemqfeeviderconfiguration) | interface | Represents the IActiveMQFeeviderConfiguration interface. | — | — |
...
- **Summary:** Represents the IActiveMQFeeviderConfiguration interface.
- **Thread safety:** Follow the lifetime and concurrency guarantees of the owning component; no additional guarantee is inferred.

**Usage recipe**

```csharp
// Resolve IActiveMQFeeviderConfiguration from the configured service container or construct it with its declared dependencies.
```
...
```mermaid
graph TD
  Current["ActiveMQ.SharedKernel"]
  Current --> T0["IActiveMQFeeviderConfiguration"]
```

The diagram shows the direct components documented by the **ActiveMQ.SharedKernel** area.

## Examples

Start with `IActiveMQFeeviderConfiguration` as the primary entry point for this folder, then follow its linked contracts and collaborators.
