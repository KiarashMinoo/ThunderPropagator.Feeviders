...
The **Feeders.GcpPubSub** area groups 3 documented types, including `GcpPubSubFeederExtensions`, `PubSubFeederConfiguration`, `PubSubFeederMessage`. It provides the contracts and implementation used by this part of ThunderPropagator.Feeviders.
...
| `GcpPubSubFeederExtensions.cs` | `GcpPubSubFeederExtensions` | 49 | Defines GcpPubSubFeederExtensions and its related behavior. |
| `PubSubFeeder.cs` | `PubSubFeeder`, `Log` | 134 | Defines PubSubFeeder, Log and its related behavior. |
| `PubSubFeederConfiguration.cs` | `PubSubFeederConfiguration` | 13 | Defines PubSubFeederConfiguration and its related behavior. |
| `PubSubFeederMessage.cs` | `PubSubFeederMessage` | 5 | Defines PubSubFeederMessage and its related behavior. |
| `PubSubMessageContext.cs` | `PubSubMessageContext` | 5 | Defines PubSubMessageContext and its related behavior. |
| `PubSubMessageSettlement.cs` | `PubSubMessageSettlement` | 24 | Defines PubSubMessageSettlement and its related behavior. |
| `ThunderPropagator.Feeders.GcpPubSub.csproj` | — | 6 | Defines project build targets, dependencies, and package metadata. |

## Types and Members

| Type | Kind | Summary | Inherits/Implements | Key Members |
|---|---|---|---|---|
| [`GcpPubSubFeederExtensions`](#gcppubsubfeederextensions) | class | Represents the GcpPubSubFeederExtensions class. | — | — |
| [`PubSubFeederConfiguration`](#pubsubfeederconfiguration) | class | Represents the PubSubFeederConfiguration class. | `AbstractFeederConfiguration, IGcpPubSubFeeviderConfiguration` | `ProjectId`, `SubscriptionId`, `MaxOutstandingMessages`, `ExactlyOnceDelivery`, `ServiceAccountKeyPath` |
| [`PubSubFeederMessage`](#pubsubfeedermessage) | class | Represents the PubSubFeederMessage class. | `FeederMessage;` | — |

### GcpPubSubFeederExtensions
...
- **Summary:** Represents the GcpPubSubFeederExtensions class.
- **Thread safety:** Follow the lifetime and concurrency guarantees of the owning component; no additional guarantee is inferred.

**Usage recipe**

```csharp
// Resolve GcpPubSubFeederExtensions from the configured service container or construct it with its declared dependencies.
```

[↑ Back to top](#contents)

### PubSubFeederConfiguration

- **Kind:** class
- **Namespace:** `ThunderPropagator.Feeders.GcpPubSub`
- **Inherits/implements:** `AbstractFeederConfiguration, IGcpPubSubFeeviderConfiguration`
- **Attributes:** None detected
- **Key members:** `ProjectId`, `SubscriptionId`, `MaxOutstandingMessages`, `ExactlyOnceDelivery`, `ServiceAccountKeyPath`
- **Summary:** Represents the PubSubFeederConfiguration class.
- **Thread safety:** Follow the lifetime and concurrency guarantees of the owning component; no additional guarantee is inferred.

**Usage recipe**

```csharp
// Resolve PubSubFeederConfiguration from the configured service container or construct it with its declared dependencies.
```
...
- **Attributes:** None detected
- **Key members:** Refer to the API surface in the source package
- **Summary:** Represents the PubSubFeederMessage class.
- **Thread safety:** Follow the lifetime and concurrency guarantees of the owning component; no additional guarantee is inferred.

**Usage recipe**

```csharp
// Resolve PubSubFeederMessage from the configured service container or construct it with its declared dependencies.
```
...
```mermaid
graph TD
  Current["Feeders.GcpPubSub"]
  Current --> T0["GcpPubSubFeederExtensions"]
  Current --> T1["PubSubFeederConfiguration"]
  Current --> T2["PubSubFeederMessage"]
```

The diagram shows the direct components documented by the **Feeders.GcpPubSub** area.

## Examples

Start with `GcpPubSubFeederExtensions` as the primary entry point for this folder, then follow its linked contracts and collaborators.

## See Also

- [Parent area](../README.md)
- [GcpPubSub.SharedKernel](../GcpPubSub.SharedKernel/README.md)
- [Providers.DotNet.GcpPubSub](../Providers.DotNet.GcpPubSub/README.md)
