...
The **UdpClient.SharedKernel** area groups 2 documented types, including `UdpClientTelemetry`, `UdpMessageProtector`. It provides the contracts and implementation used by this part of ThunderPropagator.Feeviders.
...
| `ThunderPropagator.Feeviders.UdpClient.SharedKernel.csproj` | — | 8 | Defines project build targets, dependencies, and package metadata. |
| `UdpClientTelemetry.cs` | `UdpClientTelemetry` | 37 | Defines UdpClientTelemetry and its related behavior. |
| `UdpMessageProtector.cs` | `UdpMessageProtector` | 185 | Defines UdpMessageProtector and its related behavior. |

## Types and Members

| Type | Kind | Summary | Inherits/Implements | Key Members |
|---|---|---|---|---|
| [`UdpClientTelemetry`](#udpclienttelemetry) | class | Represents the UdpClientTelemetry class. | — | — |
| [`UdpMessageProtector`](#udpmessageprotector) | class | Represents the UdpMessageProtector class. | `IDisposable` | `Protect(…)`, `Unprotect(…)` |
...
- **Summary:** Represents the UdpClientTelemetry class.
- **Thread safety:** Follow the lifetime and concurrency guarantees of the owning component; no additional guarantee is inferred.

**Usage recipe**

```csharp
// Resolve UdpClientTelemetry from the configured service container or construct it with its declared dependencies.
```
...
- **Attributes:** None detected
- **Key members:** `Protect(…)`, `Unprotect(…)`
- **Summary:** Represents the UdpMessageProtector class.
- **Thread safety:** Follow the lifetime and concurrency guarantees of the owning component; no additional guarantee is inferred.

**Usage recipe**

```csharp
// Resolve UdpMessageProtector from the configured service container or construct it with its declared dependencies.
```
...
```mermaid
graph TD
  Current["UdpClient.SharedKernel"]
  Current --> T0["UdpClientTelemetry"]
  Current --> T1["UdpMessageProtector"]
```

The diagram shows the direct components documented by the **UdpClient.SharedKernel** area.

## Examples

Start with `UdpClientTelemetry` as the primary entry point for this folder, then follow its linked contracts and collaborators.
