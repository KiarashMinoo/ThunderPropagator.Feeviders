...
The **Feeders.Grpc** area groups 3 documented types, including `GrpcFeederConfiguration`, `GrpcFeederExtensions`, `GrpcFeederMessage`. It provides the contracts and implementation used by this part of ThunderPropagator.Feeviders.
...
| `GrpcFeeder.cs` | `GrpcFeeder`, `Log` | 186 | Defines GrpcFeeder, Log and its related behavior. |
| `GrpcFeederConfiguration.cs` | `GrpcFeederConfiguration` | 22 | Defines GrpcFeederConfiguration and its related behavior. |
| `GrpcFeederExtensions.cs` | `GrpcFeederExtensions` | 75 | Defines GrpcFeederExtensions and its related behavior. |
| `GrpcFeederMessage.cs` | `GrpcFeederMessage` | 6 | Defines GrpcFeederMessage and its related behavior. |
| `GrpcReconnectDelay.cs` | `GrpcReconnectDelay` | 17 | Defines GrpcReconnectDelay and its related behavior. |
| `ThunderPropagator.Feeders.Grpc.csproj` | — | 8 | Defines project build targets, dependencies, and package metadata. |

## Types and Members

| Type | Kind | Summary | Inherits/Implements | Key Members |
|---|---|---|---|---|
| [`GrpcFeederConfiguration`](#grpcfeederconfiguration) | class | Represents the GrpcFeederConfiguration class. | `AbstractGrpcFeevidersConfiguration, IAbstractFeederConfiguration` | `Id`, `EnrichmentScript`, `MetadataReferences`, `ReceiveTimeout`, `StartupTimeout`, `MessageHandlerTimeout` |
| [`GrpcFeederExtensions`](#grpcfeederextensions) | class | Represents the GrpcFeederExtensions class. | — | — |
| [`GrpcFeederMessage`](#grpcfeedermessage) | class | Represents the GrpcFeederMessage class. | `FeederMessage;` | — |

### GrpcFeederConfiguration

- **Kind:** class
- **Namespace:** `ThunderPropagator.Feeders.Grpc`
- **Inherits/implements:** `AbstractGrpcFeevidersConfiguration, IAbstractFeederConfiguration`
...
- **Summary:** Represents the GrpcFeederConfiguration class.
- **Thread safety:** Follow the lifetime and concurrency guarantees of the owning component; no additional guarantee is inferred.

**Usage recipe**

```csharp
// Resolve GrpcFeederConfiguration from the configured service container or construct it with its declared dependencies.
```

[↑ Back to top](#contents)

### GrpcFeederExtensions
...
- **Summary:** Represents the GrpcFeederExtensions class.
- **Thread safety:** Follow the lifetime and concurrency guarantees of the owning component; no additional guarantee is inferred.

**Usage recipe**

```csharp
// Resolve GrpcFeederExtensions from the configured service container or construct it with its declared dependencies.
```
...
- **Attributes:** None detected
- **Key members:** Refer to the API surface in the source package
- **Summary:** Represents the GrpcFeederMessage class.
- **Thread safety:** Follow the lifetime and concurrency guarantees of the owning component; no additional guarantee is inferred.

**Usage recipe**

```csharp
// Resolve GrpcFeederMessage from the configured service container or construct it with its declared dependencies.
```
...
```mermaid
graph TD
  Current["Feeders.Grpc"]
  Current --> T0["GrpcFeederConfiguration"]
  Current --> T1["GrpcFeederExtensions"]
  Current --> T2["GrpcFeederMessage"]
```

The diagram shows the direct components documented by the **Feeders.Grpc** area.

## Examples

Start with `GrpcFeederConfiguration` as the primary entry point for this folder, then follow its linked contracts and collaborators.
