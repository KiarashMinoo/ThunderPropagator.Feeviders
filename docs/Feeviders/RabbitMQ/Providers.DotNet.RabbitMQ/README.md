...
The **Providers.DotNet.RabbitMQ** area groups 3 documented types, including `RabbitMQProviderConfiguration`, `RabbitMQProviderExtensions`, `RabbitMQProviderMessage`. It provides the contracts and implementation used by this part of ThunderPropagator.Feeviders.
...
| `RabbitMQProvider.cs` | `RabbitMQProvider`, `Log` | 148 | Defines RabbitMQProvider, Log and its related behavior. |
| `RabbitMQProviderConfiguration.cs` | `RabbitMQProviderConfiguration` | 16 | Defines RabbitMQProviderConfiguration and its related behavior. |
| `RabbitMQProviderExtensions.cs` | `RabbitMQProviderExtensions` | 42 | Defines RabbitMQProviderExtensions and its related behavior. |
| `RabbitMQProviderMessage.cs` | `RabbitMQProviderMessage` | 8 | Defines RabbitMQProviderMessage and its related behavior. |
| `ThunderPropagator.Providers.DotNet.RabbitMQ.csproj` | — | 8 | Defines project build targets, dependencies, and package metadata. |

## Types and Members

| Type | Kind | Summary | Inherits/Implements | Key Members |
|---|---|---|---|---|
| [`RabbitMQProviderConfiguration`](#rabbitmqproviderconfiguration) | class | Represents the RabbitMQProviderConfiguration class. | `RabbitMQFeeviderConfiguration, IAbstractProviderConfiguration` | — |
| [`RabbitMQProviderExtensions`](#rabbitmqproviderextensions) | class | Represents the RabbitMQProviderExtensions class. | — | — |
| [`RabbitMQProviderMessage`](#rabbitmqprovidermessage) | class | Represents the RabbitMQProviderMessage class. | `FeederMessage` | — |

### RabbitMQProviderConfiguration

- **Kind:** class
- **Namespace:** `ThunderPropagator.Providers.DotNet.RabbitMQ`
- **Inherits/implements:** `RabbitMQFeeviderConfiguration, IAbstractProviderConfiguration`
- **Attributes:** None detected
- **Key members:** Refer to the API surface in the source package
- **Summary:** Represents the RabbitMQProviderConfiguration class.
- **Thread safety:** Follow the lifetime and concurrency guarantees of the owning component; no additional guarantee is inferred.

**Usage recipe**

```csharp
// Resolve RabbitMQProviderConfiguration from the configured service container or construct it with its declared dependencies.
```

[↑ Back to top](#contents)

### RabbitMQProviderExtensions
...
- **Attributes:** None detected
- **Key members:** Refer to the API surface in the source package
- **Summary:** Represents the RabbitMQProviderExtensions class.
- **Thread safety:** Follow the lifetime and concurrency guarantees of the owning component; no additional guarantee is inferred.

**Usage recipe**

```csharp
// Resolve RabbitMQProviderExtensions from the configured service container or construct it with its declared dependencies.
```

[↑ Back to top](#contents)

### RabbitMQProviderMessage
...
- **Attributes:** None detected
- **Key members:** Refer to the API surface in the source package
- **Summary:** Represents the RabbitMQProviderMessage class.
- **Thread safety:** Follow the lifetime and concurrency guarantees of the owning component; no additional guarantee is inferred.

**Usage recipe**

```csharp
// Resolve RabbitMQProviderMessage from the configured service container or construct it with its declared dependencies.
```
...
```mermaid
graph TD
  Current["Providers.DotNet.RabbitMQ"]
  Current --> T0["RabbitMQProviderConfiguration"]
  Current --> T1["RabbitMQProviderExtensions"]
  Current --> T2["RabbitMQProviderMessage"]
```

The diagram shows the direct components documented by the **Providers.DotNet.RabbitMQ** area.

## Examples

Start with `RabbitMQProviderConfiguration` as the primary entry point for this folder, then follow its linked contracts and collaborators.

## See Also

- [Parent area](../README.md)
- [Feeders.RabbitMQ](../Feeders.RabbitMQ/README.md)
