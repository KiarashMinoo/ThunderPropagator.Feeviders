...
The **RabbitMQ.SharedKernel** area groups 1 documented type, including `RabbitMQFeeviderConfiguration`. It provides the contracts and implementation used by this part of ThunderPropagator.Feeviders.
...
| `RabbitMQFeeviderConfiguration.cs` | `RabbitMQFeeviderConfiguration` | 237 | Defines RabbitMQFeeviderConfiguration and its related behavior. |
| `RabbitMQFeeviderConnectionFactory.cs` | `RabbitMQFeeviderConnectionFactory` | 122 | Defines RabbitMQFeeviderConnectionFactory and its related behavior. |
| `ThunderPropagator.Feeviders.RabbitMQ.SharedKernel.csproj` | — | 15 | Defines project build targets, dependencies, and package metadata. |

## Types and Members

| Type | Kind | Summary | Inherits/Implements | Key Members |
|---|---|---|---|---|
| [`RabbitMQFeeviderConfiguration`](#rabbitmqfeeviderconfiguration) | class | Represents the RabbitMQFeeviderConfiguration class. | `ServiceConfiguration` | — |

### RabbitMQFeeviderConfiguration
...
- **Summary:** Represents the RabbitMQFeeviderConfiguration class.
- **Thread safety:** Follow the lifetime and concurrency guarantees of the owning component; no additional guarantee is inferred.

**Usage recipe**

```csharp
// Resolve RabbitMQFeeviderConfiguration from the configured service container or construct it with its declared dependencies.
```
...
```mermaid
graph TD
  Current["RabbitMQ.SharedKernel"]
  Current --> T0["RabbitMQFeeviderConfiguration"]
```

The diagram shows the direct components documented by the **RabbitMQ.SharedKernel** area.

## Examples

Start with `RabbitMQFeeviderConfiguration` as the primary entry point for this folder, then follow its linked contracts and collaborators.
