...
The **Providers.DotNet.WebApi** area groups 3 documented types, including `WebApiProviderConfiguration`, `WebApiProviderExtensions`, `WebApiProviderMessage`. It provides the contracts and implementation used by this part of ThunderPropagator.Feeviders.
...
| `ThunderPropagator.Providers.DotNet.WebApi.csproj` | — | 13 | Defines project build targets, dependencies, and package metadata. |
| `WebApiProvider.cs` | `WebApiProvider`, `Log` | 78 | Defines WebApiProvider, Log and its related behavior. |
| `WebApiProviderConfiguration.cs` | `WebApiProviderConfiguration` | 80 | Defines WebApiProviderConfiguration and its related behavior. |
| `WebApiProviderExtensions.cs` | `WebApiProviderExtensions` | 69 | Defines WebApiProviderExtensions and its related behavior. |
| `WebApiProviderMessage.cs` | `WebApiProviderMessage` | 6 | Defines WebApiProviderMessage and its related behavior. |

## Types and Members

| Type | Kind | Summary | Inherits/Implements | Key Members |
|---|---|---|---|---|
| [`WebApiProviderConfiguration`](#webapiproviderconfiguration) | class | Represents the WebApiProviderConfiguration class. | `AbstractProviderConfiguration` | — |
| [`WebApiProviderExtensions`](#webapiproviderextensions) | class | Represents the WebApiProviderExtensions class. | — | — |
| [`WebApiProviderMessage`](#webapiprovidermessage) | class | Represents the WebApiProviderMessage class. | `FeederMessage;` | — |
...
- **Summary:** Represents the WebApiProviderConfiguration class.
- **Thread safety:** Follow the lifetime and concurrency guarantees of the owning component; no additional guarantee is inferred.

**Usage recipe**

```csharp
// Resolve WebApiProviderConfiguration from the configured service container or construct it with its declared dependencies.
```

[↑ Back to top](#contents)

### WebApiProviderExtensions
...
- **Attributes:** None detected
- **Key members:** Refer to the API surface in the source package
- **Summary:** Represents the WebApiProviderExtensions class.
- **Thread safety:** Follow the lifetime and concurrency guarantees of the owning component; no additional guarantee is inferred.

**Usage recipe**

```csharp
// Resolve WebApiProviderExtensions from the configured service container or construct it with its declared dependencies.
```
...
- **Attributes:** None detected
- **Key members:** Refer to the API surface in the source package
- **Summary:** Represents the WebApiProviderMessage class.
- **Thread safety:** Follow the lifetime and concurrency guarantees of the owning component; no additional guarantee is inferred.

**Usage recipe**

```csharp
// Resolve WebApiProviderMessage from the configured service container or construct it with its declared dependencies.
```
...
```mermaid
graph TD
  Current["Providers.DotNet.WebApi"]
  Current --> T0["WebApiProviderConfiguration"]
  Current --> T1["WebApiProviderExtensions"]
  Current --> T2["WebApiProviderMessage"]
```

The diagram shows the direct components documented by the **Providers.DotNet.WebApi** area.

## Examples

Start with `WebApiProviderConfiguration` as the primary entry point for this folder, then follow its linked contracts and collaborators.

## See Also

- [Parent area](../README.md)
- [Feeders.WebApi](../Feeders.WebApi/README.md)
