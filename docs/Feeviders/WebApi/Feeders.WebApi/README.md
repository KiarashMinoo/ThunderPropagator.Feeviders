...
The **Feeders.WebApi** area groups 3 documented types, including `WebApiFeederConfiguration`, `WebApiFeederExtensions`, `WebApiFeederMessage`. It provides the contracts and implementation used by this part of ThunderPropagator.Feeviders.
...
| `ThunderPropagator.Feeders.WebApi.csproj` | — | 9 | Defines project build targets, dependencies, and package metadata. |
| `WebApiFeeder.cs` | `WebApiFeeder`, `Log` | 72 | Defines WebApiFeeder, Log and its related behavior. |
| `WebApiFeederConfiguration.cs` | `WebApiFeederConfiguration` | 13 | Defines WebApiFeederConfiguration and its related behavior. |
| `WebApiFeederExtensions.cs` | `WebApiFeederExtensions` | 102 | Defines WebApiFeederExtensions and its related behavior. |
| `WebApiFeederMessage.cs` | `WebApiFeederMessage` | 6 | Defines WebApiFeederMessage and its related behavior. |

### Direct child areas

- [Properties](./Properties/README.md) `Types:0` `Files:1`

## Types and Members

| Type | Kind | Summary | Inherits/Implements | Key Members |
|---|---|---|---|---|
| [`WebApiFeederConfiguration`](#webapifeederconfiguration) | class | Represents the WebApiFeederConfiguration class. | `AbstractFeederConfiguration` | — |
| [`WebApiFeederExtensions`](#webapifeederextensions) | class | Represents the WebApiFeederExtensions class. | — | — |
| [`WebApiFeederMessage`](#webapifeedermessage) | class | Represents the WebApiFeederMessage class. | `FeederMessage;` | — |
...
- **Summary:** Represents the WebApiFeederConfiguration class.
- **Thread safety:** Follow the lifetime and concurrency guarantees of the owning component; no additional guarantee is inferred.

**Usage recipe**

```csharp
// Resolve WebApiFeederConfiguration from the configured service container or construct it with its declared dependencies.
```

[↑ Back to top](#contents)

### WebApiFeederExtensions
...
- **Attributes:** None detected
- **Key members:** Refer to the API surface in the source package
- **Summary:** Represents the WebApiFeederExtensions class.
- **Thread safety:** Follow the lifetime and concurrency guarantees of the owning component; no additional guarantee is inferred.

**Usage recipe**

```csharp
// Resolve WebApiFeederExtensions from the configured service container or construct it with its declared dependencies.
```
...
- **Attributes:** None detected
- **Key members:** Refer to the API surface in the source package
- **Summary:** Represents the WebApiFeederMessage class.
- **Thread safety:** Follow the lifetime and concurrency guarantees of the owning component; no additional guarantee is inferred.

**Usage recipe**

```csharp
// Resolve WebApiFeederMessage from the configured service container or construct it with its declared dependencies.
```
...
```mermaid
graph TD
  Current["Feeders.WebApi"]
  Current --> C0["Properties"]
```

The diagram shows the direct components documented by the **Feeders.WebApi** area.

## Examples

Start with `WebApiFeederConfiguration` as the primary entry point for this folder, then follow its linked contracts and collaborators.

## See Also

- [Parent area](../README.md)
- [Providers.DotNet.WebApi](../Providers.DotNet.WebApi/README.md)
