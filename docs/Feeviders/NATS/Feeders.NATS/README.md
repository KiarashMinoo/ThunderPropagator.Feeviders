...
The **Feeders.NATS** area groups 3 documented types, including `NatsFeederConfiguration`, `NatsFeederExtensions`, `NatsFeederMessage`. It provides the contracts and implementation used by this part of ThunderPropagator.Feeviders.
...
| `NatsFeeder.cs` | `NatsFeeder`, `Log` | 255 | Defines NatsFeeder, Log and its related behavior. |
| `NatsFeederConfiguration.cs` | `NatsFeederConfiguration` | 109 | Defines NatsFeederConfiguration and its related behavior. |
| `NatsFeederExtensions.cs` | `NatsFeederExtensions` | 80 | Defines NatsFeederExtensions and its related behavior. |
| `NatsFeederMessage.cs` | `NatsFeederMessage` | 6 | Defines NatsFeederMessage and its related behavior. |
| `NatsJetStreamMessageSettlement.cs` | `NatsJetStreamMessageSettlement` | 71 | Defines NatsJetStreamMessageSettlement and its related behavior. |
| `ThunderPropagator.Feeders.NATS.csproj` | — | 8 | Defines project build targets, dependencies, and package metadata. |

## Types and Members

| Type | Kind | Summary | Inherits/Implements | Key Members |
|---|---|---|---|---|
| [`NatsFeederConfiguration`](#natsfeederconfiguration) | class | Represents the NatsFeederConfiguration class. | `AbstractNatsFeevidersConfiguration, IAbstractFeederConfiguration` | `ReceiveTimeout`, `StartupTimeout`, `MessageHandlerTimeout`, `MemoryPressurePauseThreshold`, `MemoryPressureResumeThreshold`, `MemoryPressurePollingInterval` |
| [`NatsFeederExtensions`](#natsfeederextensions) | class | Represents the NatsFeederExtensions class. | — | — |
| [`NatsFeederMessage`](#natsfeedermessage) | class | Represents the NatsFeederMessage class. | `FeederMessage;` | — |

### NatsFeederConfiguration

- **Kind:** class
- **Namespace:** `ThunderPropagator.Feeders.NATS`
- **Inherits/implements:** `AbstractNatsFeevidersConfiguration, IAbstractFeederConfiguration`
...
- **Summary:** Represents the NatsFeederConfiguration class.
- **Thread safety:** Follow the lifetime and concurrency guarantees of the owning component; no additional guarantee is inferred.

**Usage recipe**

```csharp
// Resolve NatsFeederConfiguration from the configured service container or construct it with its declared dependencies.
```

[↑ Back to top](#contents)

### NatsFeederExtensions
...
- **Summary:** Represents the NatsFeederExtensions class.
- **Thread safety:** Follow the lifetime and concurrency guarantees of the owning component; no additional guarantee is inferred.

**Usage recipe**

```csharp
// Resolve NatsFeederExtensions from the configured service container or construct it with its declared dependencies.
```
...
- **Attributes:** None detected
- **Key members:** Refer to the API surface in the source package
- **Summary:** Represents the NatsFeederMessage class.
- **Thread safety:** Follow the lifetime and concurrency guarantees of the owning component; no additional guarantee is inferred.

**Usage recipe**

```csharp
// Resolve NatsFeederMessage from the configured service container or construct it with its declared dependencies.
```
...
```mermaid
graph TD
  Current["Feeders.NATS"]
  Current --> T0["NatsFeederConfiguration"]
  Current --> T1["NatsFeederExtensions"]
  Current --> T2["NatsFeederMessage"]
```

The diagram shows the direct components documented by the **Feeders.NATS** area.

## Examples

Start with `NatsFeederConfiguration` as the primary entry point for this folder, then follow its linked contracts and collaborators.

## See Also

- [Parent area](../README.md)
- [NATS.SharedKernel](../NATS.SharedKernel/README.md)
