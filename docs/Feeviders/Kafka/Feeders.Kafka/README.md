...
The **Feeders.Kafka** area groups 3 documented types, including `KafkaFeederConfiguration`, `KafkaFeederExtensions`, `KafkaFeederMessage`. It provides the contracts and implementation used by this part of ThunderPropagator.Feeviders.
...
| `BlockingOperationRunner.cs` | `BlockingOperationRunner` | 16 | Defines BlockingOperationRunner and its related behavior. |
| `KafkaDeserializer.cs` | `KafkaDeserializer` | 34 | Defines KafkaDeserializer and its related behavior. |
| `KafkaFeeder.cs` | `KafkaFeeder`, `Log` | 244 | Defines KafkaFeeder, Log and its related behavior. |
| `KafkaFeederConfiguration.cs` | `KafkaFeederConfiguration` | 130 | Defines KafkaFeederConfiguration and its related behavior. |
| `KafkaFeederExtensions.cs` | `KafkaFeederExtensions` | 78 | Defines KafkaFeederExtensions and its related behavior. |
| `KafkaFeederInitializer.cs` | `KafkaFeederInitializer` | 40 | Defines KafkaFeederInitializer and its related behavior. |
| `KafkaFeederMessage.cs` | `KafkaFeederMessage` | 6 | Defines KafkaFeederMessage and its related behavior. |
| `ThunderPropagator.Feeders.Kafka.csproj` | — | 15 | Defines project build targets, dependencies, and package metadata. |

## Types and Members

| Type | Kind | Summary | Inherits/Implements | Key Members |
|---|---|---|---|---|
| [`KafkaFeederConfiguration`](#kafkafeederconfiguration) | class | Represents the KafkaFeederConfiguration class. | `ConsumerConfig,` | — |
| [`KafkaFeederExtensions`](#kafkafeederextensions) | class | Represents the KafkaFeederExtensions class. | — | — |
| [`KafkaFeederMessage`](#kafkafeedermessage) | class | Represents the KafkaFeederMessage class. | `FeederMessage;` | — |

### KafkaFeederConfiguration

- **Kind:** class
- **Namespace:** `ThunderPropagator.Feeders.Kafka`
- **Inherits/implements:** `ConsumerConfig,`
- **Attributes:** None detected
- **Key members:** Refer to the API surface in the source package
- **Summary:** Represents the KafkaFeederConfiguration class.
- **Thread safety:** Follow the lifetime and concurrency guarantees of the owning component; no additional guarantee is inferred.

**Usage recipe**

```csharp
// Resolve KafkaFeederConfiguration from the configured service container or construct it with its declared dependencies.
```

[↑ Back to top](#contents)

### KafkaFeederExtensions
...
- **Attributes:** None detected
- **Key members:** Refer to the API surface in the source package
- **Summary:** Represents the KafkaFeederExtensions class.
- **Thread safety:** Follow the lifetime and concurrency guarantees of the owning component; no additional guarantee is inferred.

**Usage recipe**

```csharp
// Resolve KafkaFeederExtensions from the configured service container or construct it with its declared dependencies.
```
...
- **Attributes:** None detected
- **Key members:** Refer to the API surface in the source package
- **Summary:** Represents the KafkaFeederMessage class.
- **Thread safety:** Follow the lifetime and concurrency guarantees of the owning component; no additional guarantee is inferred.

**Usage recipe**

```csharp
// Resolve KafkaFeederMessage from the configured service container or construct it with its declared dependencies.
```
...
```mermaid
graph TD
  Current["Feeders.Kafka"]
  Current --> T0["KafkaFeederConfiguration"]
  Current --> T1["KafkaFeederExtensions"]
  Current --> T2["KafkaFeederMessage"]
```

The diagram shows the direct components documented by the **Feeders.Kafka** area.

## Examples

Start with `KafkaFeederConfiguration` as the primary entry point for this folder, then follow its linked contracts and collaborators.

## See Also

- [Parent area](../README.md)
- [Providers.DotNet.Kafka](../Providers.DotNet.Kafka/README.md)
