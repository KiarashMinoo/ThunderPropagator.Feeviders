...
The **Providers.DotNet.TcpSocket** area groups 3 documented types, including `TcpSocketProviderConfiguration`, `TcpSocketProviderExtensions`, `TcpSocketProviderMessage`. It provides the contracts and implementation used by this part of ThunderPropagator.Feeviders.
...
| `TcpSocketProvider.cs` | `TcpSocketProvider`, `Log` | 191 | Defines TcpSocketProvider, Log and its related behavior. |
| `TcpSocketProviderConfiguration.cs` | `TcpSocketProviderConfiguration` | 57 | Defines TcpSocketProviderConfiguration and its related behavior. |
| `TcpSocketProviderExtensions.cs` | `TcpSocketProviderExtensions` | 27 | Defines TcpSocketProviderExtensions and its related behavior. |
| `TcpSocketProviderMessage.cs` | `TcpSocketProviderMessage` | 6 | Defines TcpSocketProviderMessage and its related behavior. |
| `TcpSocketSendLock.cs` | `TcpSocketSendLock` | 25 | Defines TcpSocketSendLock and its related behavior. |
| `ThunderPropagator.Providers.DotNet.TcpSocket.csproj` | — | 9 | Defines project build targets, dependencies, and package metadata. |

## Types and Members

| Type | Kind | Summary | Inherits/Implements | Key Members |
|---|---|---|---|---|
| [`TcpSocketProviderConfiguration`](#tcpsocketproviderconfiguration) | class | Represents the TcpSocketProviderConfiguration class. | `AbstractProviderConfiguration,` | — |
| [`TcpSocketProviderExtensions`](#tcpsocketproviderextensions) | class | Represents the TcpSocketProviderExtensions class. | — | — |
| [`TcpSocketProviderMessage`](#tcpsocketprovidermessage) | class | Represents the TcpSocketProviderMessage class. | `FeederMessage;` | — |
...
- **Summary:** Represents the TcpSocketProviderConfiguration class.
- **Thread safety:** Follow the lifetime and concurrency guarantees of the owning component; no additional guarantee is inferred.

**Usage recipe**

```csharp
// Resolve TcpSocketProviderConfiguration from the configured service container or construct it with its declared dependencies.
```

[↑ Back to top](#contents)

### TcpSocketProviderExtensions
...
- **Attributes:** None detected
- **Key members:** Refer to the API surface in the source package
- **Summary:** Represents the TcpSocketProviderExtensions class.
- **Thread safety:** Follow the lifetime and concurrency guarantees of the owning component; no additional guarantee is inferred.

**Usage recipe**

```csharp
// Resolve TcpSocketProviderExtensions from the configured service container or construct it with its declared dependencies.
```

[↑ Back to top](#contents)

### TcpSocketProviderMessage
...
- **Attributes:** None detected
- **Key members:** Refer to the API surface in the source package
- **Summary:** Represents the TcpSocketProviderMessage class.
- **Thread safety:** Follow the lifetime and concurrency guarantees of the owning component; no additional guarantee is inferred.

**Usage recipe**

```csharp
// Resolve TcpSocketProviderMessage from the configured service container or construct it with its declared dependencies.
```
...
```mermaid
graph TD
  Current["Providers.DotNet.TcpSocket"]
  Current --> T0["TcpSocketProviderConfiguration"]
  Current --> T1["TcpSocketProviderExtensions"]
  Current --> T2["TcpSocketProviderMessage"]
```

The diagram shows the direct components documented by the **Providers.DotNet.TcpSocket** area.

## Examples

Start with `TcpSocketProviderConfiguration` as the primary entry point for this folder, then follow its linked contracts and collaborators.
