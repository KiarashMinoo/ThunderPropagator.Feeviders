...
The **Pulsar.SharedKernel** area groups 1 documented type, including `AbstractPulsarFeevidersConfiguration`. It provides the contracts and implementation used by this part of ThunderPropagator.Feeviders.

## Files

| File | Primary type(s)/symbol(s) | LOC (approx.) | Responsibility |
|---|---|---:|---|
| `AbstractPulsarFeevidersConfiguration.cs` | `AbstractPulsarFeevidersConfiguration` | 76 | Defines AbstractPulsarFeevidersConfiguration and its related behavior. |
| `AssemblyInfo.cs` | — | 8 | Contains the assembly info implementation or configuration. |
| `JsonSchema.cs` | `JsonSchema` | 36 | Defines JsonSchema and its related behavior. |
| `PulsarClientFactory.cs` | `PulsarClientFactory` | 49 | Defines PulsarClientFactory and its related behavior. |
| `ThunderPropagator.Feeviders.Pulsar.SharedKernel.csproj` | — | 14 | Defines project build targets, dependencies, and package metadata. |

## Types and Members

| Type | Kind | Summary | Inherits/Implements | Key Members |
|---|---|---|---|---|
| [`AbstractPulsarFeevidersConfiguration`](#abstractpulsarfeevidersconfiguration) | class | Represents the AbstractPulsarFeevidersConfiguration class. | `ServiceConfiguration` | — |
...
- **Summary:** Represents the AbstractPulsarFeevidersConfiguration class.
- **Thread safety:** Follow the lifetime and concurrency guarantees of the owning component; no additional guarantee is inferred.

**Usage recipe**

```csharp
// Resolve AbstractPulsarFeevidersConfiguration from the configured service container or construct it with its declared dependencies.
```
...
```mermaid
graph TD
  Current["Pulsar.SharedKernel"]
  Current --> T0["AbstractPulsarFeevidersConfiguration"]
```

The diagram shows the direct components documented by the **Pulsar.SharedKernel** area.

## Examples

Start with `AbstractPulsarFeevidersConfiguration` as the primary entry point for this folder, then follow its linked contracts and collaborators.
