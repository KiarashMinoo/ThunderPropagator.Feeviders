...
The **Feeders.SharedKernel** area groups 2 documented types, including `Extensions`, `IFormatSerializer`. It provides the contracts and implementation used by this part of ThunderPropagator.Feeviders.

## Files

| File | Primary type(s)/symbol(s) | LOC (approx.) | Responsibility |
|---|---|---:|---|
| `AssemblyInfo.cs` | — | 22 | Contains the assembly info implementation or configuration. |
| `Extensions.cs` | `Extensions` | 35 | Defines Extensions and its related behavior. |
| `FormatSerializerInvoker.cs` | `IFormatSerializer` | 5 | Defines IFormatSerializer and its related behavior. |
| `InFlightMessageTracker.cs` | `InFlightMessageTracker` | 52 | Defines InFlightMessageTracker and its related behavior. |
| `ThunderPropagator.Feeders.SharedKernel.csproj` | — | 9 | Defines project build targets, dependencies, and package metadata. |

## Types and Members

| Type | Kind | Summary | Inherits/Implements | Key Members |
|---|---|---|---|---|
| [`Extensions`](#extensions) | class | Represents the Extensions class. | — | `AddFormatSerializerInvoker(…)` |
| [`IFormatSerializer`](#iformatserializer) | delegate | Represents the IFormatSerializer delegate. | — | — |
...
- **Key members:** `AddFormatSerializerInvoker(…)`
- **Summary:** Represents the Extensions class.
- **Thread safety:** Follow the lifetime and concurrency guarantees of the owning component; no additional guarantee is inferred.

**Usage recipe**

```csharp
// Resolve Extensions from the configured service container or construct it with its declared dependencies.
```
...
- **Summary:** Represents the IFormatSerializer delegate.
- **Thread safety:** Follow the lifetime and concurrency guarantees of the owning component; no additional guarantee is inferred.
...
```mermaid
graph TD
  Current["Feeders.SharedKernel"]
  Current --> T0["Extensions"]
  Current --> T1["IFormatSerializer"]
```

The diagram shows the direct components documented by the **Feeders.SharedKernel** area.

## Examples

Start with `Extensions` as the primary entry point for this folder, then follow its linked contracts and collaborators.

## See Also

- [Parent area](../README.md)
- [Providers.DotNet.SharedKernel](../Providers.DotNet.SharedKernel/README.md)
