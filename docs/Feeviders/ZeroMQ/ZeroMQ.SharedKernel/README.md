...
The **ZeroMQ.SharedKernel** area groups 2 documented types, including `AbstractZeroMqFeevidersConfiguration`, `ZeroMqSocketPattern`. It provides the contracts and implementation used by this part of ThunderPropagator.Feeviders.

## Files

| File | Primary type(s)/symbol(s) | LOC (approx.) | Responsibility |
|---|---|---:|---|
| `AbstractZeroMqFeevidersConfiguration.cs` | `AbstractZeroMqFeevidersConfiguration` | 17 | Defines AbstractZeroMqFeevidersConfiguration and its related behavior. |
| `AssemblyInfo.cs` | — | 9 | Contains the assembly info implementation or configuration. |
| `ThunderPropagator.Feeviders.ZeroMQ.SharedKernel.csproj` | — | 14 | Defines project build targets, dependencies, and package metadata. |
| `ZeroMqEnvelopeTranslator.cs` | `ZeroMqEnvelopeTranslator` | 49 | Defines ZeroMqEnvelopeTranslator and its related behavior. |
| `ZeroMqSocketFactory.cs` | `ZeroMqSocketFactory` | 42 | Defines ZeroMqSocketFactory and its related behavior. |
| `ZeroMqSocketPattern.cs` | `ZeroMqSocketPattern` | 14 | Defines ZeroMqSocketPattern and its related behavior. |

## Types and Members

| Type | Kind | Summary | Inherits/Implements | Key Members |
|---|---|---|---|---|
| [`AbstractZeroMqFeevidersConfiguration`](#abstractzeromqfeevidersconfiguration) | class | Represents the AbstractZeroMqFeevidersConfiguration class. | `ServiceConfiguration` | `IsEnabled`, `Endpoint`, `SocketPattern`, `Bind`, `HighWatermark`, `Linger` |
| [`ZeroMqSocketPattern`](#zeromqsocketpattern) | enum | The ZeroMQ socket pattern pair used by a Feeder/Provider. | — | — |

### AbstractZeroMqFeevidersConfiguration

- **Kind:** class
- **Namespace:** `ThunderPropagator.Feeviders.ZeroMQ.SharedKernel`
- **Inherits/implements:** `ServiceConfiguration`
- **Attributes:** None detected
- **Key members:** `IsEnabled`, `Endpoint`, `SocketPattern`, `Bind`, `HighWatermark`, `Linger`, `SerializerType`
- **Summary:** Represents the AbstractZeroMqFeevidersConfiguration class.
- **Thread safety:** Follow the lifetime and concurrency guarantees of the owning component; no additional guarantee is inferred.

**Usage recipe**

```csharp
// Resolve AbstractZeroMqFeevidersConfiguration from the configured service container or construct it with its declared dependencies.
```
...
- **Summary:** The ZeroMQ socket pattern pair used by a Feeder/Provider.
- **Thread safety:** Follow the lifetime and concurrency guarantees of the owning component; no additional guarantee is inferred.

**Usage recipe**

```csharp
// Resolve ZeroMqSocketPattern from the configured service container or construct it with its declared dependencies.
```
...
```mermaid
graph TD
  Current["ZeroMQ.SharedKernel"]
  Current --> T0["AbstractZeroMqFeevidersConfiguration"]
  Current --> T1["ZeroMqSocketPattern"]
```

The diagram shows the direct components documented by the **ZeroMQ.SharedKernel** area.

## Examples

Start with `AbstractZeroMqFeevidersConfiguration` as the primary entry point for this folder, then follow its linked contracts and collaborators.
