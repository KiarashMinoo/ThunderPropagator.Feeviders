...
The **Feeders.ZeroMQ** area groups 3 documented types, including `ZeroMqFeederConfiguration`, `ZeroMqFeederExtensions`, `ZeroMqFeederMessage`. It provides the contracts and implementation used by this part of ThunderPropagator.Feeviders.
...
| `ThunderPropagator.Feeders.ZeroMQ.csproj` | — | 8 | Defines project build targets, dependencies, and package metadata. |
| `ZeroMqFeeder.cs` | `ZeroMqFeeder`, `Log` | 156 | Defines ZeroMqFeeder, Log and its related behavior. |
| `ZeroMqFeederConfiguration.cs` | `ZeroMqFeederConfiguration` | 21 | Defines ZeroMqFeederConfiguration and its related behavior. |
| `ZeroMqFeederExtensions.cs` | `ZeroMqFeederExtensions` | 75 | Defines ZeroMqFeederExtensions and its related behavior. |
| `ZeroMqFeederMessage.cs` | `ZeroMqFeederMessage` | 6 | Defines ZeroMqFeederMessage and its related behavior. |

## Types and Members

| Type | Kind | Summary | Inherits/Implements | Key Members |
|---|---|---|---|---|
| [`ZeroMqFeederConfiguration`](#zeromqfeederconfiguration) | class | Represents the ZeroMqFeederConfiguration class. | `AbstractZeroMqFeevidersConfiguration, IAbstractFeederConfiguration` | `Id`, `EnrichmentScript`, `MetadataReferences`, `ReceiveTimeout`, `StartupTimeout`, `MessageHandlerTimeout` |
| [`ZeroMqFeederExtensions`](#zeromqfeederextensions) | class | Represents the ZeroMqFeederExtensions class. | — | — |
| [`ZeroMqFeederMessage`](#zeromqfeedermessage) | class | Represents the ZeroMqFeederMessage class. | `FeederMessage;` | — |

### ZeroMqFeederConfiguration

- **Kind:** class
- **Namespace:** `ThunderPropagator.Feeders.ZeroMQ`
- **Inherits/implements:** `AbstractZeroMqFeevidersConfiguration, IAbstractFeederConfiguration`
- **Attributes:** None detected
- **Key members:** `Id`, `EnrichmentScript`, `MetadataReferences`, `ReceiveTimeout`, `StartupTimeout`, `MessageHandlerTimeout`, `MemoryPressurePauseThreshold`, `MemoryPressureResumeThreshold`
- **Summary:** Represents the ZeroMqFeederConfiguration class.
- **Thread safety:** Follow the lifetime and concurrency guarantees of the owning component; no additional guarantee is inferred.

**Usage recipe**

```csharp
// Resolve ZeroMqFeederConfiguration from the configured service container or construct it with its declared dependencies.
```

[↑ Back to top](#contents)

### ZeroMqFeederExtensions
...
- **Summary:** Represents the ZeroMqFeederExtensions class.
- **Thread safety:** Follow the lifetime and concurrency guarantees of the owning component; no additional guarantee is inferred.

**Usage recipe**

```csharp
// Resolve ZeroMqFeederExtensions from the configured service container or construct it with its declared dependencies.
```
...
- **Attributes:** None detected
- **Key members:** Refer to the API surface in the source package
- **Summary:** Represents the ZeroMqFeederMessage class.
- **Thread safety:** Follow the lifetime and concurrency guarantees of the owning component; no additional guarantee is inferred.

**Usage recipe**

```csharp
// Resolve ZeroMqFeederMessage from the configured service container or construct it with its declared dependencies.
```
...
```mermaid
graph TD
  Current["Feeders.ZeroMQ"]
  Current --> T0["ZeroMqFeederConfiguration"]
  Current --> T1["ZeroMqFeederExtensions"]
  Current --> T2["ZeroMqFeederMessage"]
```

The diagram shows the direct components documented by the **Feeders.ZeroMQ** area.

## Examples

Start with `ZeroMqFeederConfiguration` as the primary entry point for this folder, then follow its linked contracts and collaborators.

## See Also

- [Parent area](../README.md)
- [Providers.DotNet.ZeroMQ](../Providers.DotNet.ZeroMQ/README.md)
- [ZeroMQ.SharedKernel](../ZeroMQ.SharedKernel/README.md)
