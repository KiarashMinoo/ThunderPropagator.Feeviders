...
The **Providers.DotNet.ZeroMQ** area groups 3 documented types, including `ZeroMqProviderConfiguration`, `ZeroMqProviderExtensions`, `ZeroMqProviderMessage`. It provides the contracts and implementation used by this part of ThunderPropagator.Feeviders.
...
| `ThunderPropagator.Providers.DotNet.ZeroMQ.csproj` | — | 8 | Defines project build targets, dependencies, and package metadata. |
| `ZeroMqProvider.cs` | `ZeroMqProvider`, `Log` | 128 | Defines ZeroMqProvider, Log and its related behavior. |
| `ZeroMqProviderConfiguration.cs` | `ZeroMqProviderConfiguration` | 11 | Defines ZeroMqProviderConfiguration and its related behavior. |
| `ZeroMqProviderExtensions.cs` | `ZeroMqProviderExtensions` | 35 | Defines ZeroMqProviderExtensions and its related behavior. |
| `ZeroMqProviderMessage.cs` | `ZeroMqProviderMessage` | 6 | Defines ZeroMqProviderMessage and its related behavior. |

## Types and Members

| Type | Kind | Summary | Inherits/Implements | Key Members |
|---|---|---|---|---|
| [`ZeroMqProviderConfiguration`](#zeromqproviderconfiguration) | class | Represents the ZeroMqProviderConfiguration class. | `AbstractZeroMqFeevidersConfiguration, IAbstractProviderConfiguration` | `Topic` |
| [`ZeroMqProviderExtensions`](#zeromqproviderextensions) | class | Represents the ZeroMqProviderExtensions class. | — | — |
| [`ZeroMqProviderMessage`](#zeromqprovidermessage) | class | Represents the ZeroMqProviderMessage class. | `FeederMessage;` | — |

### ZeroMqProviderConfiguration

- **Kind:** class
- **Namespace:** `ThunderPropagator.Providers.DotNet.ZeroMQ`
- **Inherits/implements:** `AbstractZeroMqFeevidersConfiguration, IAbstractProviderConfiguration`
- **Attributes:** None detected
- **Key members:** `Topic`
- **Summary:** Represents the ZeroMqProviderConfiguration class.
- **Thread safety:** Follow the lifetime and concurrency guarantees of the owning component; no additional guarantee is inferred.

**Usage recipe**

```csharp
// Resolve ZeroMqProviderConfiguration from the configured service container or construct it with its declared dependencies.
```

[↑ Back to top](#contents)

### ZeroMqProviderExtensions
...
- **Summary:** Represents the ZeroMqProviderExtensions class.
- **Thread safety:** Follow the lifetime and concurrency guarantees of the owning component; no additional guarantee is inferred.

**Usage recipe**

```csharp
// Resolve ZeroMqProviderExtensions from the configured service container or construct it with its declared dependencies.
```

[↑ Back to top](#contents)

### ZeroMqProviderMessage
...
- **Attributes:** None detected
- **Key members:** Refer to the API surface in the source package
- **Summary:** Represents the ZeroMqProviderMessage class.
- **Thread safety:** Follow the lifetime and concurrency guarantees of the owning component; no additional guarantee is inferred.

**Usage recipe**

```csharp
// Resolve ZeroMqProviderMessage from the configured service container or construct it with its declared dependencies.
```
...
```mermaid
graph TD
  Current["Providers.DotNet.ZeroMQ"]
  Current --> T0["ZeroMqProviderConfiguration"]
  Current --> T1["ZeroMqProviderExtensions"]
  Current --> T2["ZeroMqProviderMessage"]
```

The diagram shows the direct components documented by the **Providers.DotNet.ZeroMQ** area.

## Examples

Start with `ZeroMqProviderConfiguration` as the primary entry point for this folder, then follow its linked contracts and collaborators.

## See Also

- [Parent area](../README.md)
- [Feeders.ZeroMQ](../Feeders.ZeroMQ/README.md)
